...
| 명령어 | 설명 | 권한 |
| --- | --- | --- |
| `/홍보설정 [채널] [대기시간]` | 자동 게시 채널과 사용자별 재신청 대기시간 설정 | 서버 관리 |
| `/홍보신청 [제목] [내용] [링크] [이미지]` | 설정된 채널에 신청자 정보와 함께 홍보 즉시 게시 | 모든 멤버 |
| `/홍보삭제` | 내가 올린 최근 홍보를 목록에서 선택해 삭제 | 모든 멤버 |

홍보 채널에서 `@everyone`의 **메시지 보내기** 권한을 끄면 일반 잡담은 차단되고 봇이 게시한 홍보만 남습니다. `/홍보설정`의 대기시간을 생략하면 기본 24시간이며, 0으로 설정하면 제한이 없습니다.
