...
| 메시지 메뉴 `홍보로 등록` | 평소처럼 작성한 메시지를 홍보 채널로 그대로 전달 | 모든 멤버 |
| `/홍보신청 [제목] [내용] [링크] [이미지]` | 입력한 내용을 이미지 1장의 정돈된 홍보 카드로 게시 | 모든 멤버 |
| `/홍보삭제` | 내가 올린 최근 홍보를 목록에서 선택해 삭제 | 모든 멤버 |

홍보할 글과 사진을 평소처럼 한 메시지로 작성한 뒤, 해당 메시지를 우클릭하거나 길게 눌러 **앱 → 홍보로 등록**을 선택하세요. 별도 입력창 없이 원래 메시지의 글·줄바꿈·링크·첨부 파일이 그대로 전달되며, 누가 등록했는지도 게시물 위에 표시됩니다.

일정한 형식의 홍보 카드가 필요하면 `/홍보신청`을 사용하세요. 제목과 내용은 필수이며 링크와 이미지 1장은 선택 사항입니다.

`probe`, `admin` 역할 보유자와 서버 소유자·Discord 관리자 권한 보유자는 다른 사용자의 메시지도 홍보로 등록할 수 있으며, `/홍보삭제`에서 모든 사용자의 홍보를 관리할 수 있습니다. 역할 이름은 대소문자를 구분하지 않습니다.

홍보 채널에서 `@everyone`의 **메시지 보내기** 권한을 끄면 일반 잡담은 차단되고 전달된 홍보만 남습니다. 홍보 재신청 시간 제한은 없습니다.
