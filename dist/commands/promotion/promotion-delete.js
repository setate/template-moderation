...
async function deletePromotionMessages(interaction, post) {
    const channel = await interaction.client.channels.fetch(post.channelId);
    if (!channel?.isTextBased() || channel.isDMBased())
        return true;
    let mainMessageMissing = false;
    const messageIds = [post.messageId, post.noticeMessageId].filter((id) => Boolean(id));
...
async function syncRecentPromotionPosts(interaction, hasAdminAccess) {
    ...
    try {
        ...
        for (const message of messages.values()) {
            ...
            if (!promoterId)
                continue;
            if (!hasAdminAccess && promoterId !== interaction.user.id)
                continue;
