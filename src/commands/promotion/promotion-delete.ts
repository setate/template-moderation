...
import { formatPromotionDisplayName } from "../../utils/promotion-display";
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(
            posts.map((post, index) =>
                `**${index + 1}. ${post.title}**\n${hasAdminAccess ? `홍보자: ${formatPromotionDisplayName(members?.get(post.userId)?.displayName || post.userId)} · ` : ""}${formatDate(post.createdAt)} · [게시물 보기](${messageUrl(post)})`
            ).join("\n\n").slice(0, 4000)
        );
